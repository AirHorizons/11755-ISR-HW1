...
LEN_WAVEFORM = 32000 * 10

local_config = {
	'batch_size': 1,
	'eps': 1e-5,
	'sample_rate': 32000,
	'load_size': LEN_WAVEFORM,
	'name_scope': 'PaSST_PT',
	'phase': 'extract',
}
def load_audio(audio_path, sample_rate=32000, mono=True):
    # By default, librosa will resample the signal to 32000Hz(sr=None). And range in (-1., 1.)
...
def preprocess(raw_audio, config=local_config):
    ...
    assert np.max(raw_audio) <= 1, "Audio max value beyond 1"
    assert np.min(raw_audio) >= -1, "Audio min value beyond -1"

    # Shape for network is 1 x DIM x 1 x 1
    raw_audio = np.reshape(raw_audio, [1, -1])
...
def parse_args():
    ...
    parser.add_argument('--input_dir', type=str, default='mp3-32k',
                        help='Directory with the audio files to extract SoundNet feats')

    parser.add_argument('--output_dir', type=str, default='passt',
    ...
    # parser.add_argument("--feat_layer",type=str,default="conv7",
...
def extract_features(args):
    ...
    for sound_sample, audio_path in gen_audio_from_dir(args.input_dir,
        ...
        new_sample = torch.from_numpy(sound_sample).cuda()
        # raise RuntimeError("check sample size {}".format(new_sample.size()))
